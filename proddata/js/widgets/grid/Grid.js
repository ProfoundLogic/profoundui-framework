...
pui.Grid = function() {
  ...
  function importArrIntoMultiSort(arr, matches, getIsDescending) {
    ...
    for (var i = 0; i < adjArr.length; i++) {
      ...
      for (var col = 0; col < headerRow.length; col++) {
        ...
        if (matches(adjArr[i], hcell)) {
          hcell.sortDescending = getIsDescending(adjArr[i], hcell);
  ...
  function sortColumnUsingSQL(cell, restoringOrAvoidXHR) {
    ...
    if (cell != null) {
      // Bypass the columns with user defined column IDs and custom sql queries.
      var bump = bumpUserDefinedColumnIds(cell.columnId);

      clientSortColumnId = cell.columnId + bump;
      ...
      me.sortBy = cell.columnId + 1 + bump + (desc ? " DESC" : "");
    }
    else {
      ...
      for (i = 0; i < sortMultiOrder.length; i++) {
        ...
        me.sortBy += comma + (sortMultiOrder[i].columnId + 1 + bump) + (desc ? " DESC" : "");
        comma = ", ";
      }
    }
    me.sorted = true;
      ...
      req["onready"] = function(req) {
        ...
        if (response) {
          ...
          if (data.length >= 1) {
            ...
            var userDefinedDataFields = getUserDefinedDataFields(me.runtimeChildren);

            var fieldOrder = []; // Array of field names to be in the same order as the columns.
            for (var field in response["results"][0]) {
              // If the field is a not user defined data field, then add it to the fieldOrder array.
              if (userDefinedDataFields.indexOf(field) == -1) {
...
pui.BaseGrid.getPropertiesModel = function() {
  ...
  var model = [{ name: "Identification", category: true },
...

    {
      ...
      validDataTypes: ["char"],
      helpDefault: "bind",
      help: "Specifies a response data structure to be received by your program for server-side filtering. Use only for page-at-a-time grids, not for database-driven grids nor load-all grids. " +
