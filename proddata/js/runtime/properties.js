...
function getPropertiesModel() {
  ...
  cachedPropertiesModel = [
    ...
    { name: "response", format: "1 / 0", readOnly: true, hideFormatting: true, validDataTypes: ["indicator", "char", "zoned"], help: "Specifies a response indicator that is returned to your program when the element is clicked.", controls: ["button", "styled button", "graphic button", "hyperlink", "image", "css button", "icon"], context: "dspf" },
    ...
    { name: "upload response", readOnly: true, hideFormatting: true, validDataTypes: ["char"], help: "Specifies a data structure response field to be populated when files are uploaded.", controls: ["file upload","file upload dnd"], context: "dspf" },
    ...
    { name: "redirect to destination", choices: ["true", "false"], type: "boolean", hideFormatting: true, validDataTypes: ["indicator", "expression"], help: "If set to \"true\" the browser will navigate to the \"destination url\", passing the \"destination parameters\". Otherwise, the screen will submit by Ajax call. The default value is \"false\".", controls: ["button", "styled button", "graphic button", "hyperlink", "image", "css button"], context: "dspf", viewdesigner: true },      
    ...
    { name: "text transform", stylename: "textTransform", choices: ["capitalize", "uppercase", "lowercase", "none"], help: "Transforms the default formatting of the text. <br><br> Please note, this only affects how the information is displayed in the browser. If you want the user interface to send transformed values to your backend program, you must also set the Text Transform attribute on the Binding Dialog for the value being sent. <br><br>Capitalize(first character only), UPPERCASE(all), lowercase(all)." },
    ...
    { name: "icon position", choices: ["left", "right"], help: "Specifies the position of the icon.  If a position is not provided, icons other than image icons will not be displayed.", controls: ["css button", "graphic button"] },
    { name: "icon", type: "icon", help: "Identifies the icon to display in the position specified by the \"icon position\" property. Setting this property overrides the \"image source\" property. Note for browsers that Internet Explorer 9 and lower, the Material Icon set will not display.", controls: ["css button", "graphic button", "icon"] },
    ...
    { name: "date format", choices: ["MM/DD/YY", "MM/DD/YYYY", "DD/MM/YY", "DD/MM/YYYY", "DD.MM.YY", "DD.MM.YYYY", "MMDDYY", "MMDDYYYY", "DDMMYY", "DDMMYYYY", "YYMMDD", "YY/MM/DD", "YYYYMMDD", "YYYY-MM-DD", "YYYY/MM/DD"], help: "Defines the date format that is returned from the pop-up calendar.", controls: ["date field"], context: "genie" },
    ...
    { name: "image source", type: "image", attribute: "src", help: "Specifies the path to an image for an image or graphic button.", controls: ["image", "graphic button"] }, 
    ...
    { name: "allow dup key", choices: ["true", "false"], type: "boolean" , help: "This property determines whether the Dup key (Shift-Insert by default) can be used on this element.  It represents the DUP DDS keyword.", controls: ["date field", "textbox"], context: "dspf", hideFormatting: true, validDataTypes: ["indicator", "expression"], viewdesigner: false },
    { name: "dup key response", format: "1 / 0", readOnly: true, hideFormatting: true, validDataTypes: ["indicator"], help: "Specifies a response indicator that is returned to your program when the Dup key is used on this element.", controls: ["date field", "textbox"], context: "dspf", viewdesigner: false },
    ...
    { name: "allow spellcheck", attribute: "spellcheck", choices: ["true", "false"], help: "Set to false to disable the browser's built-in spellcheck for this widget.", hideFormatting: true, validDataTypes: ["indicator", "expression"], controls: ["textbox", "text area"]},
    ...
    { name: "required", choices: ["true", "false"], type: "boolean", hideFormatting: true, validDataTypes: ["indicator", "expression"], help: "When set to true, the element cannot be empty.", controls: ["combo box", "date field", "file upload", "password field", "select box", "spinner", "text area", "textbox","file upload dnd"], context: "dspf"},
    ...
    { name: "chart type", choices: pui.widgets.getChartTypes, help: "Identifies the type of chart to display.  The chart type maps to a .swf flash file in the proddata/charts folder on the IFS.  If Flash is not available, HTML5 is used to render the chart.", controls: ["chart"] },
    { name: "map type", help: "Identifies the type of map to display. The available map names are given here: <br /><br /> <a target=\"_blank\" href=\"http://www.fusioncharts.com/dev/usage-guide/setup/list-of-maps.html\">List of FusionMaps files</a><br /><br />Specifiy the \"JavaScript alias\" name, with or without the \"maps/\" prefix. If this property is specified, the \"chart type\" property is ignored.", controls: ["chart"] },
    ...
    { name: "allow drag", choices: ["true", "false"], type: "boolean", validDataTypes: ["indicator", "expression"], hideFormatting: true, help: "This property determines if the element can be drag and dropped.", controls: ["html container", "output field", "image", "ajax container", "button", "graphic button", "styled button", "css button"], context: "dspf" },
    ...
    { name: "ondragstart", type: "js", help: "Initiates a client-side script when the user first starts to drag the element.  Information about the drag and drop operation is provided using the global pui.dragDropInfo object.", controls: ["html container", "output field", "image", "ajax container", "button", "graphic button", "styled button", "css button"], context: "dspf" },
    { name: "drop targets", type: "list", help: "Specifies a list of target element id's, which identify where this element can be dropped.", controls: ["html container", "output field", "image", "ajax container", "button", "graphic button", "styled button", "css button"], context: "dspf" },
    { name: "ondragenter", type: "js", help: "Initiates a client-side script when the user drags an element over a valid drop target.  Information about the drag and drop operation is provided using the global pui.dragDropInfo object.", controls: ["html container", "output field", "image", "ajax container", "button", "graphic button", "styled button", "css button"], context: "dspf" },
    { name: "ondragleave", type: "js", help: "Initiates a client-side script when the user moves an element out of a valid drop target during a drag operation.  Information about the drag and drop operation is provided using the global pui.dragDropInfo object.", controls: ["html container", "output field", "image", "ajax container", "button", "graphic button", "styled button", "css button"], context: "dspf" },
    { name: "ondrop", type: "js", help: "Initiates a client-side script when the mouse is released during a drag and drop operation.  Information about the drag and drop operation is provided using the global pui.dragDropInfo object.", controls: ["html container", "output field", "image", "ajax container", "button", "graphic button", "styled button", "css button"], context: "dspf" },
    ...
    { name: "active tab", format: "number", hideFormatting: true, validDataTypes: ["zoned"], help: "This property specifies the initial active tab on a Tab Panel. Each tab within a Tab Panel is identified by a sequential index, starting with 0 for the first tab, 1 for the second tab, and so on.  The default value for the active tab index is 0.", controls: ["tab panel"] },
    ...
    { name: "selection mode", controls: ["file upload"], bind: false, choices: ["standard", "enhanced","single"], help: "When set to \"enhanced\" the file upload control will allow for selection of multiple files at once from the browser's file selector. This is accomplished by SHIFT+Click or CNTRL+Click or CNTRL+A. When \"standard\" mode is used, the user must select each file for upload one by one. When \"single\" mode is used, only one file can be chosen. The default mode is \"standard\".<br /><br /><strong>Note: </strong>Not all browsers support \"enhanced\" mode. For example, Internet Explorer does not. In such browsers, the control will work in \"standard\" mode regardless of this setting."},
    { name: "auto submit", controls: ["file upload dnd"], choices: ["true", "false"], type: "boolean", hideFormatting: true, context: "dspf", validDataTypes: ["indicator", "expression"], help: "After dropping a file should a page submit happen automatically. The default setting is \"false\". Upload happens during submit."},
    { name: "auto upload", controls: ["file upload dnd"], choices: ["true", "false"], type: "boolean", hideFormatting: true, context: "genie", validDataTypes: ["indicator", "expression"], help: "After dropping a file should upload happen automatically. The default setting is \"false\"."},
    { name: "number of files", controls: ["file upload","file upload dnd"], format: "number", hideFormatting: true, validDataTypes: ["zoned"], help: "This property specifies the maximum number of files that can be uploaded at one time."},    
    { name: "size limit", controls: ["file upload","file upload dnd"], hideFormatting: true, validDataTypes: ["zoned"], help: "Specifies the size limit (in MB) for each file. The default value is 10."},
    { name: "target directory", controls: ["file upload","file upload dnd"], defaultDataLength: 50, hideFormatting: true, validDataTypes: ["char"], help: "This property specifies the IFS directory where uploaded files will be saved."},
    { name: "rename to", controls: ["file upload","file upload dnd"], defaultDataLength: 50, hideFormatting: true, validDataTypes: ["char"], help: "Specifies an alternate file name to be used when the uploaded file is saved.<br /><br />Note: This property is ignored when more than 1 file is uploaded."},
    ...
    { name: "allowed type", controls: ["file upload","file upload dnd"], multOccur: true, choices: ["text/plain", "application/vnd.ms-word", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/pdf", "image/gif", "image/jpg", "image/png", "Other..."], hideFormatting: true, validDataTypes: ["char"], help: "Specifies a MIME file type (as reported by the web browser) which is allowed to be uploaded. If not set, any file type will be allowed.<br /><br />To specify multiple types, right-click the property and select Add Another Allowed Type."},
    { name: "onupload", type: "js", help: "Initiates a client-side script when files are uploaded.", controls: ["file upload","file upload dnd"], context: "genie" },
    ...
    { name: "css class", type: "cssClass", multOccur: true, help: "Defines a custom cascading style sheet class to assign to the element.  To specify multiple classes, right-click the property and select Add Another CSS Class.", context: "dspf" },
    ...
    { name: "Misc", category: true },    
    { name: "css class", type: "cssClass", attribute: "class", help: "Defines a custom cascading style sheet class to assign to the element.", context: "genie" },
...
function getScreenPropertiesModel(designScreen) {
	...
	if (context == "dspf") {
  	var model = [
    	{ name: "Identification", category: true },
    	{ name: "record format name", help: "Identifies the record format that is used to access this screen from server code.", maxLength: (pui.viewdesigner || pui.nodedesigner ? null : 10), bind: false, canBeRemoved: false },
    	{ name: "description", help: "Describes the record format.", bind: false },
      ...
      { name: "no focus", choices: ["true", "false", "no focus on page"], type: "boolean", help: "This property indicates that no field should receive focus when the screen first renders. When set to \"true\", focus will not go to any input element, but key presses (such as for function keys) will still be detected by the application without any special action from the user.<br /><br />When set to \"no focus on page\", no input element will get focus, but key presses will not be detected by the application until the user clicks on a component in the screen. This setting is useful to prevent Internet Explorer from taking focus away from another open window when the screen is rendered.", hideFormatting: true, validDataTypes: ["indicator", "expression"] },
  ...
  var model = [
	...
	{ name: "detect subfile end plus", variable: "pui.genie.config.detectSubfileEndPlus", choices: ["true", "false"], type: "boolean", help: "When \"detect subfile patterns\" is selected, this flag will allow Genie to recognize a 1A output field usually containing a '+' on the end of the last row, and remove it so it will not interrupt the pattern detection. The default value for this property is true." },
    ...
    { name: "default date format", variable: "pui.genie.config.defaultDateFormat", choices: ["MM/DD/YY", "MM/DD/YYYY", "DD/MM/YY", "DD/MM/YYYY", "DD.MM.YY", "DD.MM.YYYY", "MMDDYY", "MMDDYYYY", "DDMMYY", "DDMMYYYY", "YYMMDD", "YYYYMMDD", "YYYY-MM-DD", "YYYY/MM/DD"], help: "Defines the default date format to be used in calendar controls." },
