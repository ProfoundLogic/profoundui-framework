...
function getScreenPropertiesModel(designScreen) {
	...
	if (context == "dspf") {
      ...
      { name: "onmessage", type: "js", help: "Initiates a client-side script that receives a message from the Profound.js display.screen.sendMessage() API, which allows you to partially update screen content instead of re-rendering the entire screen. The message is received in a variable named <b>message</b>." },
  ...
  var model = [
	...
	{ name: "detect subfile end plus", variable: "pui.genie.config.detectSubfileEndPlus", choices: ["true", "false"], type: "boolean", help: "When \"detect subfile patterns\" is selected, this flag will allow Genie to recognize a 1A output field ususlly containing a '+' on the end of the last row so it will not interrupt the pattern detection. The default value for this property is true." },
